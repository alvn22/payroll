...
<head>
    <?php
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            ...
            $sql = "UPDATE karyawan SET nama='$nama', nik='$nik', id_jabatan='$jabatan', id_divisi='$divisi' WHERE id_karyawan='$id_karyawan'";
                ...
                <div class="container-fluid">
                    ...
                    <form class="offset-md-1" method="POST">
                        <?php
                            ...
                            $dt = mysqli_fetch_assoc($re);
                            $id_jabatan_user = $dt['id_jabatan'];
                            $id_divisi = $dt['id_divisi'];
                        ?>
                        <div class="col-md-10 mb-3">
                            <label class="form-label fw-bold">Nama Karyawan:</label>
                            <input type="text" value="<?php echo $dt['nama'] ?>" name="nama" class="form-control shadow-none" required>
                        </div>
                        <div class="col-md-10 mb-3">
                            <label class="form-label fw-bold">NIK:</label>
                            <input type="number" value="<?php echo $dt['nik'] ?>" name="nik" class="form-control shadow-none" required>
                        </div>
                        <div class="col-md-10 mb-3">
                                    ...
                                    <?php
                                        ...
                                        $re = mysqli_query($con,$sql);
                                            while($dt = mysqli_fetch_assoc($re)){
                                                $selected = ($dt['id_jabatan'] == $id_jabatan_user) ? "selected" : "";
                                                echo "<option value='$dt[id_jabatan]' $selected>$dt[nama_jabatan]</option>";
                                            }
                                    ?>
                                </select>
                        </div>
                            ...
                            <label class="form-label fw-bold">Divisi:</label>
                                <select class="custom-select" name="divisi">
                                    <?php
                                        ...
                                        $re = mysqli_query($con,$sql);
                                            while($dt = mysqli_fetch_assoc($re)){
                                                $selected = ($dt['id_divisi'] == $id_divisi) ? "selected" : "";
                                                echo "<option value='$dt[id_divisi]' $selected>$dt[nama_divisi]</option>";
                                            }
                                    ?>
                                </select>
                        </div>
                        <div class="col-md-10">
                            <a href="karyawan.php" class="btn btn-dark">Kembali</a>
