    ...
    <?php
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST"){
            if(isset($_POST['tambah'])){
                ...
                if(!empty($ray['nik'])){
                    alert('error', 'NIK telah terdaftar!');
                } else {
                    $q = "INSERT INTO karyawan (`nama`,`nik`,`id_jabatan`,`id_divisi`) VALUES ('$nama','$nik','$id_jabatan','$id_divisi')";
        ...
        if (isset($_GET["action"]) && $_GET["action"] == "delete" && isset($_GET["id"])) {
            $id = $_GET["id"];
            $q = "DELETE FROM karyawan WHERE id_karyawan=$id";
            $r = mysqli_query($con, $q);
                        ...
                        <div class="col-xl-12">
                            <div class="card shadow mb-4">
                                <div
                                    ...
                                    <h6 class="m-0 font-weight-bold text-primary">Overview Karyawan</h6>
                                    <button class="btn btn-primary" data-toggle="modal" data-target="#karyawanModal">
                                ...
                                <div class="card-body">
                                    ...
                                    <table class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                            <?php
                                                $q = "SELECT * FROM karyawan k JOIN jabatan j ON k.id_jabatan = j.id_jabatan JOIN divisi d ON d.id_divisi = k.id_divisi ORDER BY id_karyawan ASC";
                                                $r = mysqli_query($con,$q);
                                                $i=1;
                                                if($r){
                                                    while($row = mysqli_fetch_assoc($r)){
                                                        ...
                                                        echo "<td><a href='edit_karyawan.php?id=$row[id_karyawan]' class='btn btn-sm btn-warning'>Edit</a>
                                                        <a href='karyawan.php?action=delete&id={$row['id_karyawan']}' class='btn btn-sm btn-danger'>Hapus</a></td>";
                                                        echo "</tr>";
                                                        $i++;
                                                    }
                                                }
                                    ...
                                    <?php
                                        ...
                                        $re = mysqli_query($con,$sql);
                                            while($dt = mysqli_fetch_assoc($re)){
                                                echo "<option value='$dt[id_jabatan]'>$dt[nama_jabatan]</option>";
                                            }
                                    ?>
                                </select>
                            </div>
                            <div class="col-md-12 mb-3">
                                ...
                                <select class="custom-select" name="divisi">
                                    <?php
                                        ...
                                        $re = mysqli_query($con,$sql);
                                            while($dt = mysqli_fetch_assoc($re)){
                                                echo "<option value='$dt[id_divisi]'>$dt[nama_divisi]</option>";
                                            }
                                    ?>
                                </select>
                            </div>
